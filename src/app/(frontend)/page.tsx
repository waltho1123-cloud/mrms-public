  ...
  useEffect(() => {
    fetch('/api/v1/prompts')
      ...
      .then((data) => {
        ...
        setPrompts(list);
        const defaultPrompt = list.find((p: PromptTemplateBase) => p.isDefault);
        if (defaultPrompt) setPromptTemplateId(defaultPrompt.id);
      })
      .catch(() => {
        // Prompts endpoint not ready yet
      });
  }, []);

  // SSE for task tracking
  const { isConnected: sseConnected } = useSSE({
    url: upload.taskId ? `/api/v1/tasks/${upload.taskId}/sse` : '',
    ...
    onMessage: useCallback(
      (event: { event: string; data: unknown }) => {
        ...
        if (typeof data.progressPct === 'number') {
          setTaskProgress(data.progressPct as number);
        }
        // Auto navigate on complete
        if (data.status === 'review' || data.status === 'completed') {
          setTimeout(() => {
            router.push(`/tasks/${upload.taskId}`);
          }, 1500);
        }
      },
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <Input
            label="參與者"
            placeholder="多位參與者請用逗號分隔（選填）"
            value={participants}
            onChange={(e) => setParticipants(e.target.value)}
            helperText="例如：張三, 李四, 王五"
            disabled={!!isProcessing}
          />
        </div>
        ...
        <div className="mt-8 bg-white rounded-xl border border-gray-200 p-6">
          ...
          {(taskStatus === 'review' || taskStatus === 'completed') && (
            <div className="mt-6 text-center">
              ...
              <Button
                variant="primary"
                onClick={() => router.push(`/tasks/${upload.taskId}`)}
              >
                查看會議紀錄
              </Button>
            </div>
